...
const SiloDashboard = () => {
  ...
  const temperature = useTemperature(silo)
  const humidity = useHumidity(silo)
  const pressure = usePressure(silo)

  const [capacity, capacityArr] = useCapacity(silo)
  ...
  useEffect(() => {
    ...
    const init = async() => {
      setThresholds(await getThresholds(silo))
    }
...
<Notification className="is-danger is-light">TEST ALARM NOTIFICATION TEST ALARM NOTIFICATION TEST ALARM NOTIFICATION TEST ALARM NOTIFICATION TEST ALARM NOTIFICATION TEST ALARM NOTIFICATION TEST ALARM NOTIFICATION TEST ALARM NOTIFICATION TEST ALARM NOTIFICATION TEST ALARM NOTIFICATION</Notification>
    ...
    {modalVisible && <UpdateThresholdsModal silo={silo} thresholds={thresholds} onClose={handleCloseModal}/>}
    </>
